...
def race_buy():
    ...
    # CRITICAL: This is where the race condition vulnerability exists!
    # We check the balance but don't immediately return an error
    # Instead, we proceed with the purchase logic and let the race condition happen
    
    # Simulate race condition vulnerability - intentional delay
    time.sleep(0.1)  # This delay makes the race condition exploitable
    
    # Now check balance again (this is where race condition can occur)
    if balance >= 100:
        # Update balance
        c.execute('UPDATE race_state SET balance = ?, last_purchase = ? WHERE session_id = ?',
                  (balance - 100, datetime.now(), session_id))
        conn.commit()
        conn.close()
        
        log_telemetry('flag_purchased', {'balance_before': balance, 'balance_after': balance - 100})
        
        return jsonify({'success': True, 'flag': FLAGS[6], 'balance': balance - 100})
    else:
        # Even with insufficient balance, we might succeed due to race condition
        # This is the vulnerability - multiple requests can pass the balance check
        conn.close()
